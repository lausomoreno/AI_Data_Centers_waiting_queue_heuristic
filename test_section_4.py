...
class GPUSystemParameters:
    ...
    def __init__(self):
        ...
        self.lambda_arrival = 12.0   # report1 §1: stable window λ < λ*≈18.4; was 60.0
...
class LPControlPolicy:
    ...
    def compute_control_power(self, state, current_power):
        # Aggregate counts across classes for LP (LP only needs tau,s)
        ts_counts = {}
...
def build_transition_matrix(params):
    ...
    for g, gc in enumerate(params.gpu_classes):
        ...
        # lambda*_g = N_g / mean_service_time
        # (Little's law: N_busy = lam * mean_service_time, saturates at N_g)
        N_g = gc['n_gpus']
        if mean_service_time > 0 and not np.isinf(mean_service_time):
            lam_star_g = N_g / mean_service_time
        else:
            lam_star_g = 0.0
        ...
        results[class_name] = {
            'rho':               rho,
            'lambda_star':       lam_star_g,
            'lambda_min':        lam_min_g,
        ...
        print(f"  [{class_name}] rho(A)={rho:.4f}  "
              f"E[service]={mean_service_time:.2f}  "
              f"lambda*={lam_star_g:.2f}  "
              f"lambda_min={lam_min_g:.2f}  "
              f"jobs={job_names}")

    # ── System-level bounds ───────────────────────────────────────────────────
    lam_star_system = min(r['lambda_star'] for r in results.values()
                          if r['lambda_star'] > 0)
    lam_min_system  = max(r['lambda_min']  for r in results.values()
                          if not np.isinf(r['lambda_min']))
...
def print_transition_matrix_summary(tm):
    ...
    print("=" * 65)
    print(f"{'Class':<8} {'rho(A)':>8} {'E[svc]':>8} "
          f"{'lam*_g':>8} {'lam_min_g':>10} {'Jobs'}")
    print("-" * 65)
    for name, lam_star in tm['lambda_star'].items():
        r   = tm['class_results'][name]
        print(f"  {name:<6} {r['rho']:>8.4f} {r['mean_service_time']:>8.2f} "
              f"{lam_star:>8.2f} {r['lambda_min']:>10.2f}  "
              f"{r['job_types']}")
    print("-" * 65)
    print(f"  System lambda_max (binding) = {tm['lambda_star_system']:.2f}  "
          f"[class with smallest lam*]")
    print(f"  System lambda_min           = {tm['lambda_min_system']:.2f}  "
          f"[class needing most load to reach P_target]")
    ...
    if window > 0:
        ...
        print("  => No operating point achieves both bounded queue AND full P_target tracking.")
        print("     This is the key finding motivating the MPC formulation.")
    print("=" * 65)
